...
def expert_situations(request):
    if request.user.is_authenticated():
        c = {'scenarios': Situation.objects.all(), 'sit_list': SituationType.objects.all()}
        c.update(csrf(request))
...
def add_situation(request):
    if request.user.is_authenticated():
        if 'description' in request.POST.keys():
            ...
            description = ''
        parent = SituationType.objects.get(id=int(request.POST['parent_st']))
        if 'st' in request.POST.keys() and request.POST['st']:
            new_st = SituationType.new(request.POST['name'], parent, description)
            return redirect('/expert/situations/')
        else:
            new_st = Situation.new(request.POST['name'], description, parent)
            return redirect('/expert/situations/'+str(new_st.id)+'/')
    else:
        return redirect('/expert/')
